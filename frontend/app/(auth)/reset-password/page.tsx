...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/v1/auth/reset-password/${token}?email=${encodeURIComponent(email)}`, {
        method: "POST",
        ...
        <div className="relative z-10 flex flex-col justify-end p-12 text-white">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center border border-white/20">
              <Image src="/imizi_logo.svg" alt="Imizi" width={24} height={24} />
            </div>
          ...
          <div className="bg-[#FFFDFA] rounded-3xl shadow-xl shadow-black/5 border border-[#EDE3D3] p-10">
            ...
            {success ? (
              <div className="text-center py-8">
                ...
                <h2 className="text-2xl font-serif text-[#3A2E22] mb-2">Password Reset!</h2>
                <p className="text-sm text-[#8B5E3C] mb-6">
                  Your password has been reset successfully.
                </p>
                <Link
                  href="/login"
                  className="inline-flex items-center gap-2 bg-[#4A3428] text-white px-6 py-3 rounded-xl font-medium hover:bg-[#3A2E22] active:scale-[0.98] transition-all shadow-lg shadow-[#4A3428]/20"
                >
              ...
              <>
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-serif text-[#3A2E22] mb-2">Reset Password</h2>
