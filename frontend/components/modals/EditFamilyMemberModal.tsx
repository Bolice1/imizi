...
export default function EditFamilyMemberModal({ isOpen, onClose, onSuccess, member, familyMembers = [] }: EditFamilyMemberModalProps) {
  ...
  useEffect(() => {
    if (isOpen && member) {
      setForm({
        ...
        generation: member.generation ? String(member.generation) : "3",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/family/members/${member._id}`, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#FFFDFA] rounded-3xl border border-[#EDE3D3] w-full max-w-md max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#EDE3D3]">
          <h2 className="text-lg font-semibold text-[#3A2E22]">Edit family member</h2>
