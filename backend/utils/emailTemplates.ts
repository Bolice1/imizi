...
const familyInvitationEmailTemplate = (inviterName: string, familyName: string, inviteUrl: string): string => {
    return `
  <!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>You are invited to join a family on Imizi</title>
    <style type="text/css">
      body { margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #F5EFE6; }
      table { border-collapse: collapse; }
      a { color: #8B5E3C; }
        ...
        <tr>
          <td align="center">
            <div class="email-container">
              ...
              <div class="header">
                <h1>You are invited to join ${familyName}</h1>
              </div>

              <div class="content">
                <p>Hi,</p>

                <p><strong>${inviterName}</strong> has invited you to join the family <strong>${familyName}</strong> on Imizi.</p>

                <p>Imizi helps families capture, organize, and preserve shared memories together. Click the button below to accept the invitation and start contributing.</p>

                <center>
                  <a href="${inviteUrl}" class="btn">Join Family</a>
                </center>

                <p class="link-fallback">If the button doesn't work, copy and paste this link into your browser:<br>${inviteUrl}</p>

                <p class="notice">This invitation will expire in 7 days. If you weren't expecting this invitation, you can safely ignore this email.</p>
              </div>
